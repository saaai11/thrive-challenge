...
import { of } from 'rxjs';
import { Characters, IImages, IName } from '../futuruma.interface';
...
describe('CharactersComponent', () => {
  ...
  it('should make sure that the name field is present', () => {
    const nameElement =
      fixture.debugElement.nativeElement.querySelector('.content');
    expect(nameElement.textContent).not.toBeNull();
  });

  it('should check if name is correct and displayed', () => {
    const nameElement =
      fixture.debugElement.nativeElement.querySelector('.content');
    expect(nameElement.textContent).toEqual(' Philip Jay Fry ');
  });

  it('check if router has been called', () => {
